...
class cyBot():
    allowed_chats = [-1001218498698, -1001171687704, -1001252516868]
    allowed_channels = [-1001183412743, -1001308153582, -1001428687999, -1001410198217, -1001446957910]
    ...
    def __init__(self, bot, update):
        if update and update.message or update.channel_post or update.callback_query:
            self.bot = bot
            self.update = update
            try:
        ...
        if self.update.message:
            ...
            if self.update.message.new_chat_members:
                    for member in self.update.message.new_chat_members:
                        ...
                        self.bot.sendMessage(chat_id=self.chat_id, reply_to_message_id=self.message_id, text="**welcome!\n[simple user verification]**", parse_mode="Markdown", reply_markup=reply_markup)
        
        
        if self.update.callback_query:
            self.tgCallback()
    
        if self.update.message.text and self.update.message.chat.id in self.allowed_chats:
    ...
    def tgMessage(self):
        ...
        if command == "/getme":
            ...
            if len(txt) != 2:
                return
            file = self.bot.sendDocument(chat_id=self.chat_id,reply_to_message_id=self.message_id,document=txt[1])
    ...
    def tgChannel():
        if self.update.channel_post.entities:
            post = parserEntities(self.update.channel_post.entities, self.update.channel_post.text)
            if not post:
                return
            #redditPost(post["title"], post["url"])
            #twitterPost(post)
